...
class AgentModule(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.using_utterances:
            self.utterance_processor = GoalPredictingProcessingModule(
                config.utterance_processor)
            self.utterance_pooling = nn.AdaptiveMaxPool2d(
                (1, config.feat_vec_size))
    ...
    def get_utterance_feat(self, game, agent, goal_predictions):
        if self.using_utterances:
            utterance_processes = Variable(
                self.Tensor(game.batch_size, game.num_agents,
                            self.processing_hidden_size))
    ...
    def get_action(self, game, agent, physical_feat, utterance_feat, movements,
                   utterances):
        movement, utterance, new_mem = self.action_processor(
            physical_feat, game.observed_goals[:, agent],
            game.memories["action"][:, agent], self.training, utterance_feat)
    ...
    def forward(self, game):
        timesteps = []
        if self.using_cuda:
            utters =  torch.zeros(game.batch_size, game.num_agents, self.vocab_size).cuda()
        for t in range(self.time_horizon):
            ...
            if self.using_utterances:
                ...
                goal_predictions = Variable(
                    self.Tensor(game.batch_size, game.num_agents,
                                game.num_agents, self.goal_size))
            ...
            if self.penalizing_words:
                pe_cost, prob = self.word_counter(utterances, t)
                cost = cost + pe_cost
            ...
            # num_utters = len(counter.keys())
            num_utters = len(torch.unique(indices))
        return self.total_cost, timesteps, num_utters, prob

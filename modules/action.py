...
class ActionModule(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.movement_chooser = nn.Sequential(
            nn.Linear(config.action_processor.hidden_size,
                      config.action_processor.hidden_size), nn.ELU(),
            nn.Linear(config.action_processor.hidden_size,
                      config.movement_dim_size), nn.Tanh())

        if self.using_utterances:
            self.utterance_chooser = nn.Sequential(
                nn.Linear(config.action_processor.hidden_size,
                          config.hidden_size), nn.ELU(),
    ...
    def forward(self, physical, goal, mem, training, utterance=None):
        ...
        if self.using_utterances:
            x = torch.cat(
                [physical.squeeze(1),
                 utterance.squeeze(1), goal_processed], 1).squeeze(1)
        else:
        ...
        if self.using_utterances:
            ...
            utterance = None
        final_movement = (
            movement * 2 * self.movement_step_size) - self.movement_step_size

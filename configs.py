...
TrainingConfig = NamedTuple('TrainingConfig', [('num_epochs', int),
                                               ('learning_rate', float),
                                               ('load_model', bool),
                                               ('load_model_file', str),
                                               ('save_model', bool),
...
GoalPredictingProcessingModuleConfig = NamedTuple(
    ...
    [('processor', ProcessingModuleConfig), ('hidden_size', int),
     ('dropout', float), ('goal_size', int)])
...
ActionModuleConfig = NamedTuple("ActionModuleConfig",
                                [('goal_processor', ProcessingModuleConfig),
                                 ...
                                 ('vocab_size', int), ('use_utterances', bool),
                                 ('use_cuda', bool)])

AgentModuleConfig = NamedTuple(
    "AgentModuleConfig",
    [('time_horizon', int), ('feat_vec_size', int), ('movement_dim_size', int),
     ('goal_size', int), ('vocab_size', int),
     ...
     ('word_counter', WordCountingModuleConfig), ('use_utterances', bool),
     ('penalize_words', bool), ('use_cuda', bool)])
...
default_game_config = GameConfig(
    ...
    MIN_AGENTS, MIN_LANDMARKS, NUM_SHAPES, NUM_COLORS, USE_UTTERANCES,
    DEFAULT_VOCAB_SIZE, DEFAULT_HIDDEN_SIZE, False)
...
default_agent_config = AgentModuleConfig(
    ...
    physical_processor=get_processor_config_with_input_size(
...
def get_training_config(kwargs):
    return TrainingConfig(
        ...
        or default_training_config.learning_rate,
        load_model=bool(kwargs['load_model_weights']),
        load_model_file=kwargs['load_model_weights']
        or default_training_config.load_model_file,
        save_model=default_training_config.save_model,
        save_model_file=kwargs['save_model_weights']
        or default_training_config.save_model_file,
...
def get_game_config(kwargs):
    return GameConfig(
        ...
        max_landmarks=kwargs['max_landmarks']
        or default_game_config.max_landmarks,
        min_landmarks=kwargs['min_landmarks']
        or default_game_config.min_landmarks,
...
def get_agent_config(kwargs):
    ...
    utterance_processor = GoalPredictingProcessingModuleConfig(
        ...
        goal_size=constants.GOAL_SIZE)
    action_processor = ActionModuleConfig(
        goal_processor=get_processor_config_with_input_size(
            constants.GOAL_SIZE),
    ...
    return AgentModuleConfig(
        ...
        or default_agent_config.time_horizon,
